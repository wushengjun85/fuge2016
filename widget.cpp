...
int ttyNum;
char *telnum = "4006379727";
uchar telflag;


uchar enginewarning;//发动机报警
uchar engineshutdown;//发动机停止

extern bool flagqh2;//切换标志位
extern bool BH;

extern uchar flagtest;

uchar flaglanguage; //中英文切换标志变量。
...
//        ui->label->setText(QString::number(jiyouwendu,'f',0));//机油温度
            ui->label->setText(QString::number(0,'f',0));//机油温度
...
//            }
            ...
            //卸粮筒  燃油进水
            if(youshuifenli)//xieliangtongbaijin
            {
                pixShanshuo.load("./imagejy/17.png");//.jpg
                paintShanshuo.drawPixmap(560,20,35,38,pixShanshuo);
            }
...
void Widget::on_pushButton_timesetup_2_clicked()  //打电话按钮 //查询
{
//    this->close();
//    findlook->show();
//    findlook->exec();
...
//    {
//        telflag = 0;
//        ui->pushButton_timesetup_2->setStyleSheet("QPushButton{border-image:url(./imagejy/zw/tp.png);background-repeat: background-xy;background-position: center;background-attachment: fixed;background-clip: padding}");
//    }
//    else
//    {
//        telflag = 1;
//        ui->pushButton_timesetup_2->setStyleSheet("QPushButton{border-image:url(./imagejy/zw/tplv.png);background-repeat: background-xy;background-position: center;background-attachment: fixed;background-clip: padding}");
//    }
...
/*******************************************************************/
        ...
        QSqlDatabase zydb;
        if(QSqlDatabase::contains("qt_sql_default_connection"))
          zydb = QSqlDatabase::database("qt_sql_default_connection");
        else
          zydb = QSqlDatabase::addDatabase("QSQLITE");

       zydb.setDatabaseName("my5.db");
       if (!zydb.open())
       {
           qDebug()<<"open database failed ---"<<zydb.lastError().text()<<"/n";
       }
       QSqlQuery queryzy;

       queryzy.exec("select id, name, age from zy");
       while (queryzy.next())
       {
           flagtest = queryzy.value(2).toInt();
       }

      // query.exec(QObject::tr("drop zy"));
