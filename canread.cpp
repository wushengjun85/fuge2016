...
//            flagFDJYR = yure;
             ...
             //byte1 移位操作
            SouSa = (frdup.data[0]>>7)&0x01;  //手刹指示灯（手刹指示灯+）   8,
            flagPark= SouSa;
            ...
            flagBattery   = (frdup.data[0]>>4)&0x01; // 充电指示灯- 5,


            Jinguang = (frdup.data[0]>>3)&0x01; //近光指示灯（近光指示灯）4,
            flagJG = Jinguang;

            YuanGuang = (frdup.data[0]>>2)&0x01; //远光指示灯（远光指示灯） 3,
            flagYG = YuanGuang;

            Youzhuan = (frdup.data[0]>>1)&0x01; //右转向灯（右转向灯）2,
            flagRight = Youzhuan;

            Zuozhuan = frdup.data[0]&0x01; //左转向灯（左转向灯）1,
            flagLeft = Zuozhuan;
      ...
      case  0x18FF11F6:
            ...
            shengyunqi = frdup.data[1]<<8|frdup.data[0];//升运器转速
            shengyunqi &= 0x3fff;
            duanlu_syq = (frdup.data[5]>>6)&0x03;

            futuoqi  = frdup.data[3]<<8|frdup.data[2];// 剥皮机转速
            futuoqi &= 0x3fff;
            duanlu_ftq = (frdup.data[7]>>6)&0x03;
            ...
            qiesuiqizhuansu = frdup.data[3]<<8|frdup.data[2];//切碎器转速
            qiesuiqizhuansu &= 0x3fff;
            //duanlu_qsq = (frdup.data[1]>>6)&0x03;

            guoqiaozhuansu = frdup.data[1]<<8|frdup.data[0];////喂入转速；
     ...
     case  0x18FF21F6:
       ...
       case 0x18FF31F6:
           ...
           else if(frdup.data[0]<1)
           {
                flagyouliangdi = 1; //油量低报警
                flagyouxiangman = 0;
           }
           else if((frdup.data[0]<8)&&(frdup.data[0]>1))
           {
                flagyouliangdi = 0;
                 flagyouxiangman = 0;
           }
