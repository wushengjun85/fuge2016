...
extern int ttyNum;
extern char *telnum;
extern uchar telflag;


extern bool flaglanguage; //中英文切换标志变量。

extern uchar flagtest;

extern uchar enginewarning;//发动机报警
extern uchar engineshutdown;//发动机停止




//static uchar can_err_count_work = 0; //Can 开路或者故障
//bool  can_err_flag_work = false;//Can 开路或者故障
//int   can_retern_work = 0; // can_return = ret //返回can 读取的数据
    ...
    ui(new Ui::Work)
{
    ...
    telpwork = new Telephone();//初始化打电话对象
...
void Work::paintEvent(QPaintEvent *event)
{
    ...
    if(flagtest == 0)
    {
       ...
       // qDebug()<<"flage.......7777777777777"<<endl;
    }
    else if(flagtest == 1)
    {
        dppix.load("./imagejy/zw/workwrbp.bmp");
        //qDebug()<<"flage.......99999"<<endl;
    }
    dp.drawPixmap(0,0,1024,600,dppix);

    //qDebug()<<"flage.......6666666666666"<<endl;
...
//            Cam_Power_on(4); //替代AV1
            ...
            system("echo 1 > /sys/class/gpio/gpio130/value");//V3 打开倒车
            system("echo 0 > /sys/class/gpio/gpio131/value");//V2 关闭
            //system("echo 0 > /sys/class/gpio/gpio132/value");//V4 关闭 单独的一路视频
            system("echo 0 > /sys/class/gpio/gpio14/value");//V4 关闭 单独的一路视频



            //qDebug()<<"1111111111111111111CAmer close "<<endl;
...
//             Cam_Power_off(4); //替代AV1
            ...
            system("echo 0 > /sys/class/gpio/gpio130/value");//V3 关闭倒车
            system("echo 1 > /sys/class/gpio/gpio131/value");//V2
//            system("echo 1 > /sys/class/gpio/gpio132/value");//V4 打开 单独的一路视频
            system("echo 1 > /sys/class/gpio/gpio14/value");//V4 打开 单独的一路视频

           // qDebug()<<"222222222222222222222222CAmer close "<<endl;
...
//             Cam_Power_off(4); //替代AV1
            ...
            //qDebug()<<"333333333333333333333333333CAmer close "<<endl;
...
//              //Cam_Power_on(2);
           ...
           // qDebug()<<"444444444444444444444444CAmer close "<<endl;
...
//                ui->label_6->setText(QString::number(jiyouwendu,'f',0));//机油温度
                ui->label_6->setText(QString::number(0));//机油温度
...
//                        Cam_Power_off(4);
                       ...
                       // qDebug()<<"555555555555555555555555CAmer close "<<endl;
                ...
                if(1) //左转灯闪烁
                {
                    ...
                    switch(countLeft)
                    {
                        ...
                        if(flagSW)//水温 0~120度  95度以上报警。
                        {
                             ...
                             paintShanshuo.drawPixmap(113,15,45,40,pixShanshuo);//正上方位置显示的图标
                        }

                        if(flagJY) //机油  油压报警 0～1 MPa,在0.1 MPa 以下为报警区。
                        {
                             pixShanshuo.load("./imagejy/08.png");//08.jpg
                             paintShanshuo.drawPixmap(391,22,59,24,pixShanshuo);//正上方位置显示的图标
...
//比较can传过来的值 ，实现闪烁
                  ...
                  if(can_retern != 0)
                  {
                      if (tuoliguntong||qiesuiqizhuansu||fengjizhuansu||futuoqi||shengyunqi||guoqiaozhuansu||fenliguntong)
                      {
#if 0
                        ...
                        if(tuoliguntong<ruby[0])//脱粒滚筒转速
                        {
                            ...
                            else if(flagtest == 1)
                            {
                                ...
                                paintShanshuo.drawPixmap(729,433,113,21,pixShanshuo);
                        ...
                        if(qiesuiqizhuansu<ruby[1])//切碎器转速
                        {
                            if (flagtest == 0)
                            {
                                ...
                                paintShanshuo.drawPixmap(727,343,130,20,pixShanshuo);
                            }
                            else if(flagtest == 1)
                            {
                                pixShanshuo.load("./imagejy/zw/qsq.png");//11.jpg
                                paintShanshuo.drawPixmap(735,365,94,20,pixShanshuo);
                        ...
                        if(fengjizhuansu<ruby[2])//风机转速
                        {
                            ...
                            else if(flagtest == 1)
                            {
                                ...
                                paintShanshuo.drawPixmap(736,167,75,20,pixShanshuo);
                            }
                        }

                        if(futuoqi<ruby[3])//复脱器堵塞转速  剥皮机
                        {
                            if (flagtest ==0)
                            {
                            ...
                            {
                                pixShanshuo.load("./imagejy/zw/bpj.png");//11.jpg ftq 剥皮机
                                paintShanshuo.drawPixmap(736,251,94,21,pixShanshuo);
                            }
                        }
                        if(shengyunqi<ruby[4])//升运器堵塞转速
                        {
                            ...
                            else if(flagtest == 1)
                            {
                                ...
                                paintShanshuo.drawPixmap(736,306,93,22,pixShanshuo);
                            }
                        }
                        if(guoqiaozhuansu<ruby[5])//过桥转速  喂入
                        {
                            if(flagtest == 0)
                            {
                            ...
                            {
                                pixShanshuo.load("./imagejy/zw/weiru.png");//11.jpg  喂入
                                paintShanshuo.drawPixmap(736,119,75,21,pixShanshuo);
                            }
                        }
#if 0
                        if(fenliguntong<ruby[6])//分离滚筒转速
                        {
                            ...
                            else if (flagtest == 1)
                            {
                                ...
                                paintShanshuo.drawPixmap(729,492,113,21,pixShanshuo);
                            }
                        }
#endif
                      }//end of ifif (tuoliguntong||qiesuiqizhuansu||fengjizhuansu||futuoqi||shengyunqi||guoqiaozhuansu||fenliguntong)
...
/*******************************************************************/
                    ...
                    if(flagBattery)//电瓶指示灯
                    {
                        ...
                        paintShanshuo.drawPixmap(50,15,46,32,pixShanshuo);//正上方图片显示
                    ...
                    if(flagLCM)//flagLCM = 1; //粮仓满
                    {
                        ...
                        paintShanshuo.drawPixmap(510,15,42,41,pixShanshuo);
                    }
                    ...
                    if(flagFDJYR)//flagFDJYR = 1; //发动机预热
                    {
                        ...
                        paintShanshuo.drawPixmap(737,25,46,27,pixShanshuo);
                    }

                    if(flagGL)//空 滤
                    {
                        pixShanshuo.load("./imagejy/04.png");//04.jpg
                        paintShanshuo.drawPixmap(459,15,42,43,pixShanshuo);
                    }
...
//                    }
                    ...
                    if(flagFDJGZ)//发动机故障
                    {
                        ...
                        paintShanshuo.drawPixmap(838,15,47,30,pixShanshuo);
                    }
                    ...
                    if(flagWidthlamp)//示宽灯
                    {
                        ...
                        paintShanshuo.drawPixmap(167,15,57,32,pixShanshuo);
                    }

                    if(flagJG)//近光灯flagJG
                    {
                        pixShanshuo.load("./imagejy/12.png");//12.jpg
                        paintShanshuo.drawPixmap(240,15,46,37,pixShanshuo);
                    }

                    if(flagYG)//远光灯
                    {
                        pixShanshuo.load("./imagejy/05.png");//05.jpg
                        paintShanshuo.drawPixmap(313,15,46,37,pixShanshuo);
                    }

                    if(flagPark)//停车 刹车
                    {
                        pixShanshuo.load("./imagejy/11.png");//11.jpg
                        paintShanshuo.drawPixmap(787,15,48,37,pixShanshuo);
                    }

                    if(flagYeyayouwen)//液压油温
                    {
                         pixShanshuo.load("./imagejy/10.png");//10.jpg
                         paintShanshuo.drawPixmap(678,15,48,38,pixShanshuo);//上边图标
                         //paintShanshuo.drawPixmap(423,216,49,38,pixShanshuo);//左边图标
                    }

                    //卸粮筒
                    if(xieliangtongbaijin)
                    {
                        pixShanshuo.load("./imagejy/xlt.png");//14.jpg
                        paintShanshuo.drawPixmap(560,20,43,31,pixShanshuo);
                    }


                    if(enginewarning)
                    {
                        pixShanshuo.load("./imagejy/ewarn.png");//.jpg
                        paintShanshuo.drawPixmap(895,25,32,21,pixShanshuo);
                    }

                    if(engineshutdown)
                    {
                        pixShanshuo.load("./imagejy/eshutdown.png");//.jpg
                        paintShanshuo.drawPixmap(937,25,32,21,pixShanshuo);
                    }
...
//    {
//        telflag = 0;
//        ui->pushButton_4->setStyleSheet("QPushButton{border-image:url(./imagejy/zw/tp.png);background-repeat: background-xy;background-position: center;background-attachment: fixed;background-clip: padding}");
//    }
//    else
//    {
//        telflag = 1;
//        ui->pushButton_4->setStyleSheet("QPushButton{border-image:url(./imagejy/zw/tplv.png);background-repeat: background-xy;background-position: center;background-attachment: fixed;background-clip: padding}");
//    }
...
void Work::on_pushButton_2_clicked()// 设置按钮   出现过错误 临时改掉 this->close();
{
    //this->close();
    time_work->show();
    time_work->exec();
...
void Work::workgzmslot()
{
    ...
    //qDebug()<<"can_return = "<<can_retern<<endl;
#if 0

    if ((can_retern == 0)||(can_retern == -1))
    {
       can_err_count_work++;
       if(can_err_count_work>=5)
       {
           ui->label_15->setText("CAN SIGNAL ERROR !!");
           //can_err_count = 0;
...
void Work::on_pushButton_5_clicked()//多媒体播放 按钮
{
      QMPlayer *qmplayer = new QMPlayer;
      connect(qmplayer, SIGNAL(destroyed()), this, SLOT(show()));
      //qmplayer->setAttribute(Qt::WA_DeleteOnClose);     /*2016/10/14 wangxw,delete,会导致同一个对象free两次的问题*/
      qmplayer->show();
      qmplayer->resize(1024, 600);
}
